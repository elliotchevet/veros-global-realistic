...
class GlobalOneDegreeSetup(VerosSetup):
    ...
    def set_parameter(self, state):
        ...
        #settings.restart_input_filename = "./global_1deg_0960.restart.h5"
        ...
        settings.runlen = 5*24*3600#(30 * 360 - 5) * 24 * 3600
        ...
        settings.enable_ray_friction = True
        settings.r_ray = 1e-4  # weak interior damping
    ...
    def slope_limiter(self,kbot, max_slope, n_iter):
        for _ in range(n_iter):
            kbot = npx.minimum(kbot, npx.roll(kbot, 1, axis=0) + max_slope)   # west
            kbot = npx.minimum(kbot, npx.roll(kbot, -1, axis=0) + max_slope)  # east
            kbot = npx.minimum(kbot, npx.roll(kbot, 1, axis=1) + max_slope)   # south
            kbot = npx.minimum(kbot, npx.roll(kbot, -1, axis=1) + max_slope)  # north
        return kbot
    ...
    def set_topography(self, state):
        ...
        #kbot_inner = vs.kbot[2:-2,2:-2]
        #kbot_sloped = self.slope_limiter(kbot_inner, max_slope=4, n_iter=1)
        #vs.kbot = update(vs.kbot, at[2:-2,2:-2], kbot_sloped) 
        mask_bathy = topo_z_smoothed == 0
        ...
        salt_np = npx.array(salt_data)
        kbot_np = npx.array(vs.kbot)
        
        ds = xr.Dataset(
            {
                "salt": (("x", "y", "z"), salt_np),
                "kbot": (("x", "y"), kbot_np[2:-2,2:-2]),
            },
            coords={
                "x": vs.xt[2:-2],
                "y": vs.yt[2:-2],
